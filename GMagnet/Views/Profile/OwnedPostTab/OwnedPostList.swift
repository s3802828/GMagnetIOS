...
struct OwnedPostList: View {
    ...
    var filteredPost: [Post] {
        if searchText == "" { //if not search, return all
            return profile.recent_posts.sorted(){ // sort by created date
                $0.createdAt.dateValue() > $1.createdAt.dateValue()
            }
            
        }
        return profile.recent_posts.filter { //otherwise, filter the posts whose title or content contain search text
            $0.title.lowercased()
